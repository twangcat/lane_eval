...
def main(args=None):
    ...
    #proto_fname = '/deep/group/driving_data/twangcat/caffe_results/proto/raw_outputs_bigmask/'+unique_name+'-Vshape_batch0.proto'
    ...
    for it in range(num_batches):
      ...
      #proto_fname = '/deep/group/driving_data/twangcat/caffe_results/proto/raw_outputs_bigmask/'+unique_name+'_batch'+str(it)+'.proto'
      fid = open(proto_fname,'r')
      bpv.ParseFromString(fid.read())
      fid.close()
                ...
                if 1==1:
                  ...
                  for lid in range(len(lane_conf)):
                    ...
                    for rid in range(1,l.shape[0]):
                      cv2.line(img, (int(l[rid-1, 0]),int(l[rid-1,1])), (int(l[rid,0]), int(l[rid,1])), colors[lane_ids[lid]], thickness=2)
                      cv2.line(topDown, (int(l3d[rid-1,0]*6+240), int(479-(l3d[rid-1,2]-4)*6+6)) , (int(l3d[rid,0]*6+240), int(479-(l3d[rid,2]-4)*6+6)) ,colors[lane_ids[lid]],1)
